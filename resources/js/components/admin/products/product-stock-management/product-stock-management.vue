...
import { AlertTriangle, CheckCircle, Edit, Package } from 'lucide-vue-next';
    ...
    <ui-card>
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <ui-button @click="openModal" variant="outline" size="sm" :prefix-icon="Edit" label="Update Stock" />
            </div>
        </template>

        <div class="space-y-4">
            <!-- Current Stock Display -->
            <div class="flex items-center gap-3 rounded-lg border p-3" :class="getStockStatus().bgColor">
                <component :is="getStockStatus().icon" class="h-5 w-5" :class="getStockStatus().color" />
                <div class="flex-1">
                    <p class="text-lg font-medium" :class="getStockStatus().color">
