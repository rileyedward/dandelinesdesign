...
<template>
    <div class="space-y-8">
        ...
        <div v-if="loading" class="py-16 text-center">
            <div class="inline-flex items-center rounded-full bg-blue-50 px-6 py-3 text-blue-600">
        ...
        <div v-else-if="blogPosts.length === 0" class="py-16 text-center">
            ...
            <h3 class="mb-2 text-lg font-medium text-gray-900">No blog posts yet</h3>
            <p class="text-gray-600">{{ noDataText }}</p>
        </div>

        <!-- Blog Posts Grid -->
        <div v-else class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            <blog-card v-for="post in blogPosts" :key="post.id" :post="post" />
        </div>
    </div>
</template>
