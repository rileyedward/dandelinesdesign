...
const getStatusBadge = (isPublished: boolean) => {
    ...
    if (isPublished) {
        return h('span', { class: `${classes} bg-green-100 text-green-800` }, [h(CheckCircle, { class: 'w-3 h-3 mr-1' }), 'Published']);
    } else {
        return h('span', { class: `${classes} bg-yellow-100 text-yellow-800` }, [h(XCircle, { class: 'w-3 h-3 mr-1' }), 'Draft']);
    }
};
